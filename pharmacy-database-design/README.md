...
3. Лекарства, заказанные до определенного числа (запрос с GROUP BY, LEFT OUTER JOIN).
...
4. Данные о поставках за все время (запрос с ORDER BY).
...
5. Количество лекарств, стоимость которых ниже 500 у.е. (запрос с функцией COUNT (агрегирования))
...
6. Названия лекарства, его стоимость, страна производителя и название фирмы поставщика (запрос с INNER JOIN).
...
11. Количество товарных накладных по каждому клиенту (таблицы, связанные более чем по одному полю).
...
13. Сотрудники, их должности и оклады (запрос с использование CROSS JOIN).
...
15. Стоимость Супрастина повысилась на 20%.
